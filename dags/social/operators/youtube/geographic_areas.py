...
class YoutubeGeographicOperator(BaseOperator, GCSDataUpload):
    ...
    def _get_data_and_upload(self,ds):
        ...
        response= self.hook.get_geographic_areas(channel_id=self.channel_id, metrics=self.metrics, ds= (datetime.strptime(ds, '%Y-%m-%d') - timedelta(days=3)).date())
