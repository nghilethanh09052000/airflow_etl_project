...
class FacebookPageImpressionOperator(BaseOperator, GCSDataUpload):
    ...
    def _get_data_and_upload(self, ds):
        ...
        data = self.hook.get_page_impression_gender_locate(page_id=self.page_id, metrics=self.metrics, ds= (datetime.strptime(ds, '%Y-%m-%d') - timedelta(days=1)).date())
        print(data)
        self.upload(
