...
from sensor_tower.operators import ActiveUsersEndpointOperator
...
def group_tasks_get_daily_and_monthly_active_users_data_on_specific_app_ids(
        ds: str,
        group_task_id: str,
        gcs_bucket: str,
        gcs_prefix: str,
    ...
    with TaskGroup(
        group_task_id,
        tooltip="Get Monthly And Daily Active User Data Based On Specific App ID in All Countries"
    ) as group_task_id:
    
        tasks = []
        for time_period in ['day', 'month']:
                        
            task = ActiveUsersEndpointOperator(
                ds=ds,
                task_id=f'get_active_user_data_{time_period}',
